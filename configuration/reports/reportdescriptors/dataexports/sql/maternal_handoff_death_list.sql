...
from person p 
 inner join visit v on v.patient_id = p.person_id and v.date_started <= p.death_date and date(v.date_stopped) >= date(p.death_date) 
where p.death_date >= DATE_SUB(@handoverDate, INTERVAL 1 YEAR) + INTERVAL 8 HOUR
and p.death_date <  @handoverDate + INTERVAL 8 HOUR
 and exists 
